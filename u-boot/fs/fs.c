...
#if defined(IMG_ENC)
	if(strstr(filename, ".itb") != NULL)
	{
		mbedtls_aes_context aes;

#if defined(IMG_ENC_LOCATION_DEV)
		unsigned char key[32] = {	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
#elif defined(IMG_ENC_LOCATION_KEYFILE)
		unsigned char key[32] = {	0x43, 0x36, 0x35, 0xDA, 0x66, 0x70, 0xE9, 0xE6,
						0x8E, 0x8F, 0xC0, 0x99, 0x4E, 0x4D, 0xAD, 0x33,
						0xF1, 0xD9, 0xF5, 0x53, 0x82, 0xFE, 0x6D, 0x6E,
						0xD9, 0xD6, 0xC5, 0x0F, 0x61, 0x59, 0x00, 0xCF};
#else
#error IMG_ENC_KSRC is not well defined
#endif
		...
		int cbc_ret = mbedtls_aes_crypt_cbc(&aes, MBEDTLS_AES_DECRYPT, len_read, iv, (unsigned char*) addr, (unsigned char*) addr);
	    printf("file name:%s, imageBase:%p, imageSize:%d, cbc_ret = 0x%s%x\n",filename, (void *)addr, (unsigned int)len_read, cbc_ret<0?"-":"", cbc_ret<0?-(unsigned)cbc_ret:cbc_ret);
	}
#endif
