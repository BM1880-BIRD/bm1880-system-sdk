...
def usb_send_file(filename, dest_addr, delay_ms):
    ...
    while (complete_cnt < 1): # For stress test
        ...
        while (content_size > 0):
            ...
            #    header.append(BM_USB_KEEP_DL)
            #else:
            header.append(BM_USB_NONE)
...
def protocol_msg_s2d_once(addr, dataBuf, length):
    msg = []
    protocol_msg_fill_header(msg, BM_USB_S2D, addr, USB_MSG_S2D_SIZE, length)
...
def protocol_msg_d2s_once(addr, length):
    msg = []
    protocol_msg_fill_header(msg, BM_USB_D2S, addr, USB_MSG_D2S_SIZE, length)
...
if __name__ == '__main__':
    ...
    while test_cnt > 0:
        ...
        if protocol_msg_s2d(target_address, data_w, copy_size) == FAIL:
            usb_send_req(BM_USB_BREAK, 0, 8, 0)
            sys.exit(-1)
        # read data
