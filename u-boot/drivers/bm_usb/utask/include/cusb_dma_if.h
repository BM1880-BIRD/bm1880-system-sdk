...
struct CUSBDMA_OBJ {
	 ...
	 */
	uint32_t (*probe)(struct CUSBDMA_Config *config, struct CUSBDMA_SysReq *sysReq);
	...
	uint32_t (*init)(void *pD, const struct CUSBDMA_Config *config,
			 struct CUSBDMA_Callbacks *callbacks);
	 ...
	 *           transferred/received by ADMA module and offset that indicates
	 ...
	 * @return DMA_STATUS_UNKNOWN if channel is unallocated
	 ...
	 * @return EINVAL if illegal/inconsistent values in parameters
	 */
	enum CUSBDMA_Status (*getChannelStatus)(void *pD, void *channel);
