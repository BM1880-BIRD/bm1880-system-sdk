 ...
 * bytes issued but not transferred by hardware because of aborted
...
struct CUSBD_Config {
	...
	enum CUSBD_DMAInterfaceWidth dmaInterfaceWidth;
	/**
	 ...
	 * PRECISE_BURST_128
	 */
	enum CUSBD_PreciseBurstLength preciseBurstLength;
...
struct CUSBD_OBJ {
	 ...
	 */
	uint32_t (*probe)(struct CUSBD_Config *config, struct CUSBD_SysReq *sysReq);
	/**
	 ...
	 */
	uint32_t (*init)(void *pD, const struct CUSBD_Config *config, struct CUSBD_Callbacks *callbacks);
	/**
	 ...
	 */
	uint32_t (*epEnable)(void *pD, struct CUSBD_Ep *ep, const struct CH9_UsbEndpointDescriptor *desc);
	/**
	 * Disable Endpoint. Functions unconfigures hardware endpoint.
	 * Endpoint will not respond to any host packets. This function
	 * should be called within SET_CONFIGURATION(configNum = 0) request
	 ...
	 */
	uint32_t (*epDisable)(void *pD, struct CUSBD_Ep *ep);
	/**
	 ...
	 * @return EINVAL if pD or ep is NULL
	 */
	uint32_t (*epSetHalt)(void *pD, struct CUSBD_Ep *ep, uint8_t value);
	/**
	 * Set halt on endpoint. Function sets endpoint to permanent halt
	 ...
	 * @return EINVAL if pD or ep is NULL
	 */
	uint32_t (*epSetWedge)(void *pD, struct CUSBD_Ep *ep);
	/**
	 ...
	 * data bytes issued but not transferred by hardware because of
	 ...
	 * @return EINVAL if pD or ep is NULL
	 */
	uint32_t (*epFifoStatus)(void *pD, struct CUSBD_Ep *ep);
	/**
	 * Flush hardware endpoint FIFO.
	 * @param[in] ep endpoint which FIFO is to be flushed
	 * @param[in] pD driver state info specific to this instance
	 */
	void (*epFifoFlush)(void *pD, struct CUSBD_Ep *ep);
	/**
	 * Submits IN/OUT transfer request to an endpoint.
	 ...
	 */
	uint32_t (*reqQueue)(void *pD, struct CUSBD_Ep *ep, struct CUSBD_Req *req);
	/**
	 ...
	 * @return EINVAL if pD or ep is NULL
	 */
	uint32_t (*reqDequeue)(void *pD, struct CUSBD_Ep *ep, struct CUSBD_Req *req);
	/**
	 ...
	 */
	uint32_t (*reqAlloc)(void *pD, struct CUSBD_Ep *ep, struct CUSBD_Req **req);
	/**
	 * Frees memory occupied by request. While operating, when request
	 * object will be no longer used, use reqFree function to free system
	 * memory *
	 ...
	 * @param[in] pD driver state info specific to this instance
	 */
	void (*reqFree)(void *pD, struct CUSBD_Ep *ep, struct CUSBD_Req *req);
	/**
	 ...
	 * @param[in] pD driver state info specific to this instance
	 */
	void (*getDevInstance)(void *pD, struct CUSBD_Dev **dev);
	/**
	 ...
	 * @param[in] pD driver state info specific to this instance
	 */
	void (*dGetConfigParams)(void *pD, struct CH9_ConfigParams *configParams);
};
 ...
 */
extern struct CUSBD_OBJ *CUSBD_GetInstance(void);
