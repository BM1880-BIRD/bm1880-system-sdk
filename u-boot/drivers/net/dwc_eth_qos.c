...
#ifdef CONFIG_TARGET_BITMAIN_BM1880_ASIC_MINER
/* set eth0 to rmii, (reference clock from phy to 1880) */
	writel(0x000000CC, 0x50010034);
#elif defined(CONFIG_DWC_ETH_2_RGMII_PORT)
/* set eth0/eth1 both to rgmii */
	writel(0x00000011, 0x50010034);
#else
/* set eth0 to rgmii, eth1 to rmii(reference clock from phy to 1880) */
	writel(0x000000C1, 0x50010034);
#endif
#ifdef CONFIG_TARGET_BITMAIN_BM1880_FPGA
	/* add timing constraint for the TX/RX of ethernet, only used for bm1880 FPGA*/
	writel(0x00000102, 0x50010078);
#endif

	/* Set pinmux for Ethernet 1*/
	val = (readl(0x50010474) & ~(0x00300000));
	writel(0x00100000 | val, 0x50010474); /*Set RGMII1_TXD0*/

	val = (readl(0x50010478) & ~(0x00300030));
	writel(0x00100010 | val, 0x50010478); /*Set RGMII1_TXD1, RGMII1_TXD2 */

	val = (readl(0x5001047C) & ~(0x00300030));
	writel(0x00100010 | val, 0x5001047C); /*Set RGMII1_TXCTL, RGMII1_TXD3 */

	val = (readl(0x50010480) & ~(0x00300030));
	writel(0x00100010 | val, 0x50010480); /*Set RGMII1_RXD1, RGMII1_RXD1 */

	val = (readl(0x50010484) & ~(0x00300030));
	writel(0x00100010 | val, 0x50010484); /*Set RGMII1_RXD2, RGMII1_RXD3 */

	val = (readl(0x50010488) & ~(0x00300030));
	writel(0x00100010 | val, 0x50010488); /*Set RGMII1_RXCTRLC, RGMII1_TXC */

	val = (readl(0x5001048C) & ~(0x00300030));
	writel(0x00100010 | val, 0x5001048C); /*Set RGMII1_RXREF, RGMII1_RXC */

	val = (readl(0x50010490) & ~(0x00300030));
	writel(0x00100010 | val, 0x50010490); /*Set RGMII1_MDC, RGMII1_IRQ */

	val = (readl(0x50010494) & ~(0x00000030));
	writel(0x00000010 | val, 0x50010494); /*Set RGMII1_MDIO  */

#endif /* CONFIG_TARGET_BITMAIN_BM1880 */
	/* reset dma */
