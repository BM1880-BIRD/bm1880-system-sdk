...
detect_thread(void *arg) {
  ...
  for (size_t m = 0; m < images_all.size(); m++) {
    ...
    for (size_t i = 0; i < faceInfo.size(); i++) {
      ...
      cout << facePts.x[0] << " " << facePts.y[0] << "," << endl;
      cout << facePts.x[1] << " " << facePts.y[1] << "," << endl;
      cout << facePts.x[2] << " " << facePts.y[2] << "," << endl;
      cout << facePts.x[3] << " " << facePts.y[3] << "," << endl;
      cout << facePts.x[4] << " " << facePts.y[4] << "," << endl;
      cout << "\r\n" << endl;
    }
#ifdef SAVE_IMAGE
...
int main(int argc, char **argv)
{
  ...
  string det1(argv[1]), det2(argv[1]);
  det1.append(SSH_MODEL_CFG_FILE);
  det2.append(ONET_MODEL_CFG_FILE);

  if (!FileExist(det1) || !FileExist(det2)) {
