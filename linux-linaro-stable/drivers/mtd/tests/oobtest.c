...
static void dump_buf(char *s, void *buf, int len)
{
	int i;
	int line_counter = 0;
	int sep_flag = 0;
...
static int write_eraseblock(int ebnum)
{
	...
	pr_info("use_len %d\n", use_len);
	pr_info("use_offset %d\n", use_offset);
	pr_info("use_len_max %d\n", use_len_max);
...
//		dump_buf("ops.oobbuf", ops.oobbuf, ops.ooblen);
...
static int verify_all_eraseblocks(void)
{
	...
	pr_info("verify_all_eraseblocks\n");
	for (i = 0; i < ebcnt; ++i) {
		if (bbt[i])
			continue;
