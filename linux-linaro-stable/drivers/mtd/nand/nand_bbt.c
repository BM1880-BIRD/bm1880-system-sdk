		...
		for (i = 0; i < len; i++) {
			...
			for (j = 0; j < 8; j += bits, act++) {
				...
				if (reserved_block_code && (tmp == reserved_block_code)) {
					pr_info("read_bbt: reserved block at 0x%012llx\n",
						 (loff_t)(offs + act) <<
...
static int search_bbt(struct mtd_info *mtd, uint8_t *buf, struct nand_bbt_descr *td)
{
	...
	for (i = 0; i < chips; i++) {
		if (td->pages[i] == -1)
			pr_info("Bad block table not found for chip %d\n", i);
		else
			pr_info("Bad block table found at page 0x%04x, version 0x%02X\n",
