...
#define REG_DWCEQOS_DMA_CH0_TXDESC_LIST_H 0x1110
#define REG_DWCEQOS_DMA_CH0_TXDESC_LIST   0x1114
#define REG_DWCEQOS_DMA_CH0_RXDESC_LIST_H 0x1118
...
#define DWCEQOS_DMA_SYSBUS_MODE_EAME     BIT(11)
...
#ifdef CONFIG_ARCH_BITMAIN
/*
 * ARM64 cacheline size(128B) is beyond the mac dma descriptor skip
 * (only 3 bits for skip, so support max 44B)
 */
};
#else
...
#ifdef DWCEQOS_ADDRWIDTH_40
	lp->rx_descs[index].des0 = lower_32_bits(new_skb_baddr);
	lp->rx_descs[index].des1 = upper_32_bits(new_skb_baddr);
#else
...
#ifdef CONFIG_ARCH_BITMAIN
/* no REG_DWCEQOS_MAC_1US_TIC_COUNTER reg */
...
#ifdef DWCEQOS_ADDRWIDTH_40
	dwceqos_write(lp, REG_DWCEQOS_DMA_CH0_TXDESC_LIST_H,
		      0x01);
#endif
...
#ifdef DWCEQOS_ADDRWIDTH_40
	dwceqos_write(lp, REG_DWCEQOS_DMA_CH0_RXDESC_LIST_H,
		      0x01);
#endif
...
#ifdef DWCEQOS_ADDRWIDTH_40
	dd->des0 = lower_32_bits(dma_handle);
#else
	dd->des0 = dma_handle;
#endif
...
#ifdef DWCEQOS_ADDRWIDTH_40
		dd->des1 = upper_32_bits(dma_handle);
#else
		dd->des1 = 0;
#endif
...
#ifdef DWCEQOS_ADDRWIDTH_40
			dd->des0 = lower_32_bits(dma_handle + consumed_size);
			dd->des1 = upper_32_bits(dma_handle);
#else
