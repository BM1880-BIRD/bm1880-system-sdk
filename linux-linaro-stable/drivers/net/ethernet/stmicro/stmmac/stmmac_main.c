...
#ifdef DWMAC_ADDRWIDTH_40
		desc->des1 = cpu_to_le32(upper_32_bits(des + (total_len - tmp_len)));
		desc->des0 = cpu_to_le32(lower_32_bits(des + (total_len - tmp_len)));
#else
...
#ifdef DWMAC_ADDRWIDTH_40
	first->des1 = cpu_to_le32(upper_32_bits(des));
	first->des0 = cpu_to_le32(lower_32_bits(des));

	tmp_pay_len = pay_len;

	stmmac_tso_allocator(priv, des + proto_hdr_len, tmp_pay_len,
				(nfrags == 0), queue);
#else
...
#ifdef DWMAC_ADDRWIDTH_40
			desc->des1 = cpu_to_le32(upper_32_bits(des));
			desc->des0 = cpu_to_le32(lower_32_bits(des));
#else
...
static inline void stmmac_rx_refill(struct stmmac_priv *priv, u32 queue)
{
	...
	while (dirty-- > 0) {
		...
		if (likely(!rx_q->rx_skbuff[entry])) {
			...
			if (unlikely(priv->synopsys_id >= DWMAC_CORE_4_00)) {
#ifdef DWMAC_ADDRWIDTH_40
				p->des0 = cpu_to_le32(lower_32_bits(rx_q->rx_skbuff_dma[entry]));
