...
class bm_usb_pyserial:
        ...
        while (connect == -1):
            ...
            try:
                ...
                if timeout != 0 and ((time.time() - query_time) > timeout):
                    ...
                    sys.stdout.flush()
                    sys.exit(-1)
                sys.stdout.write("Waiting for BM1880 USB connect: %s    \r" % (progress_symbol[i%len(progress_symbol)]))
                sys.stdout.flush()
                time.sleep(0.05)
    ...
    def usb_send_file(self, filename, dest_addr, delay_ms):
        ...
        while (complete_cnt < 1): # For stress test
            ...
            while (content_size > 0):
                ...
                if filename == 'bm_dl_magic.bin':
                    self.header.append(pkt.BM_USB_KEEP_DL)
                else:
                    self.header.append(pkt.BM_USB_NONE)
    ...
    def protocol_msg_s2d_once(self, addr, dataBuf, length):
        msg = []
        self.protocol_msg_fill_header(msg, pkt.BM_USB_S2D, addr, pkt.USB_MSG_S2D_SIZE, length)
